...
class ItemStorage{
    ...
    public function get_by_price(int $price_from, int $price_to){
      ...
      $buff=[];
      foreach($this->items as $elem){
        if($elem->getPrice()>=$price_from && $elem->getPrice()<=$price_to){
          $buff[]=$elem;
        }
      }
      return $buff;
    ...
    public function get_by_material(...$materials){
      ...
      $buff=[];
      foreach ($materials as $elemMaterial){
        foreach ($this->items as $elem){
          if ($elem->getMaterial()==$elemMaterial){
            $buff[]=$elem;
          }
        }
      }
    ...
    public function get_by_price_and_material(int $price_to, string $material){
      ...
      $buff=[];
      foreach ($this->items as $elem){
        if ($elem->getMaterial()==$material && $elem->getPrice()<=$price_to){
          $buff[]=$elem;
        }
      }
      return $buff;
